...
def plotar_grafico(df_t, ticker, etf_nome):
    """Gráfico interativo Plotly — suporta pinch-to-zoom, pan e scroll no mobile."""
    close  = df_t['Close']
    ema20  = df_t.get('EMA20')
    ema50  = df_t.get('EMA50')
    ema200 = df_t.get('EMA200')
    bb_low = df_t.get('BB_Lower')
    bb_up  = df_t.get('BB_Upper')
    rsi_s  = df_t.get('RSI14')
    st_s   = df_t.get('Stoch_K')
    ...
    e20v   = float(ema20.iloc[-1])  if ema20  is not None and len(ema20)  > 0 else None
    e50v   = float(ema50.iloc[-1])  if ema50  is not None and len(ema50)  > 0 else None
    e200v  = float(ema200.iloc[-1]) if ema200 is not None and len(ema200) > 0 else None
    rsi_v  = float(rsi_s.iloc[-1])  if rsi_s  is not None and len(rsi_s)  > 0 else 50
    st_v   = float(st_s.iloc[-1])   if st_s   is not None and len(st_s)   > 0 else 50
    is_val = ((100 - rsi_v) + (100 - st_v)) / 2
    ...
    if bb_low is not None and bb_up is not None:
        fig.add_trace(go.Scatter(
            x=bb_up.index, y=bb_up,
            line=dict(color='rgba(150,150,150,0.4)', width=1, dash='dot'),
            showlegend=False, hoverinfo='skip',
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=bb_low.index, y=bb_low,
            line=dict(color='rgba(150,150,150,0.4)', width=1, dash='dot'),
            fill='tonexty', fillcolor='rgba(150,150,150,0.08)',
            showlegend=False, hoverinfo='skip',
        ), row=1, col=1)

    # Fibonacci linhas + anotações
    for nivel, pf in fib_levels.items():
        cor = fib_colors[nivel]
        fig.add_hline(
            y=pf, line_dash='dash', line_color=cor, line_width=1,
            opacity=0.65,
            annotation_text=f' Fib {nivel}',
    ...
    fig.add_trace(go.Scatter(
        ...
        name='Close', line=dict(color='#1E1E1E', width=2),
    ), row=1, col=1)

    # EMAs
    if ema20  is not None: fig.add_trace(go.Scatter(x=ema20.index,  y=ema20,  name='EMA20',  line=dict(color='#2962FF', width=1.5), opacity=0.9), row=1, col=1)
    if ema50  is not None: fig.add_trace(go.Scatter(x=ema50.index,  y=ema50,  name='EMA50',  line=dict(color='#FF6D00', width=1.5), opacity=0.8), row=1, col=1)
    if ema200 is not None: fig.add_trace(go.Scatter(x=ema200.index, y=ema200, name='EMA200', line=dict(color='#00695C', width=2.0), opacity=0.7), row=1, col=1)

    # ── Painel 2: RSI ──────────────────────────────────────────────────────────
    if rsi_s is not None:
        fig.add_trace(go.Scatter(
            x=rsi_s.index, y=rsi_s, name='RSI14',
            line=dict(color='#FF6F00', width=1.5),
        ), row=2, col=1)
        fig.add_hrect(y0=0,  y1=30,  fillcolor='rgba(244,67,54,0.12)',  line_width=0, row=2, col=1)
        fig.add_hrect(y0=70, y1=100, fillcolor='rgba(76,175,80,0.12)',  line_width=0, row=2, col=1)
    ...
    if st_s is not None:
        fig.add_trace(go.Scatter(
            x=st_s.index, y=st_s, name='Stoch %K',
            line=dict(color='#9C27B0', width=1.5),
        ), row=3, col=1)
        fig.add_hrect(y0=0,  y1=20,  fillcolor='rgba(244,67,54,0.12)',  line_width=0, row=3, col=1)
        fig.add_hrect(y0=80, y1=100, fillcolor='rgba(76,175,80,0.12)',  line_width=0, row=3, col=1)
    ...
    fig.update_layout(
        ...
        paper_bgcolor='white',
        plot_bgcolor='#fafafa',
        font=dict(family='sans-serif', size=11, color='#1E1E1E'),
        legend=dict(orientation='h', y=1.06, x=0, font=dict(size=10)),
        margin=dict(l=50, r=90, t=60, b=20),
        hovermode='x unified',
        # Interatividade: zoom com pinch, scroll e arraste
        dragmode='zoom',
    ...
    fig.update_xaxes(
        showgrid=True, gridcolor='rgba(0,0,0,0.06)',
        # Range slider no painel 3 para navegação rápida
        rangeslider=dict(visible=False),
    )
    # Range selector no eixo X (botões de período rápido)
    fig.update_xaxes(
        rangeselector=dict(
            buttons=[
    ...
    fig.update_yaxes(title_text='Stoch %K',    row=3, col=1, range=[0, 100])
