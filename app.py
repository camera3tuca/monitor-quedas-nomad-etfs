...
def plotar_grafico(df_t, ticker, etf_nome):
    ...
    close  = df_t['Close']
    ema20  = df_t['EMA20']  if 'EMA20'  in df_t.columns else None
    ema50  = df_t['EMA50']  if 'EMA50'  in df_t.columns else None
    ema200 = df_t['EMA200'] if 'EMA200' in df_t.columns else None
    bb_low = df_t['BB_Lower'] if 'BB_Lower' in df_t.columns else None
    bb_up  = df_t['BB_Upper'] if 'BB_Upper' in df_t.columns else None
    rsi_s  = df_t['RSI14']   if 'RSI14'   in df_t.columns else None
    st_s   = df_t['Stoch_K'] if 'Stoch_K' in df_t.columns else None
    ...
    # ── Status de tendência ────────────────────────────────────────────────────
    ultimo = close.iloc[-1]
    e20v   = ema20.iloc[-1]  if ema20  is not None else None
    e50v   = ema50.iloc[-1]  if ema50  is not None else None
    e200v  = ema200.iloc[-1] if ema200 is not None else None
    if e20v and e50v and e200v:
        if ultimo > e20v > e50v > e200v: status = "🟢 Tendência Forte de Alta"
        elif ultimo > e20v and ultimo > e50v and ultimo > e200v: status = "🟢 Acima das 3 EMAs"
        elif ultimo < e20v and ultimo < e50v and ultimo < e200v: status = "🔴 Abaixo das 3 EMAs"
        else: status = "🟡 Tendência Mista"
    elif e20v:
        status = "🟢 Acima EMA20" if ultimo > e20v else "🔴 Abaixo EMA20"
    else:
        status = ""

    # Nível Fibonacci mais próximo
    nivel_prox = min(fib_levels, key=lambda n: abs(ultimo - fib_levels[n]))
    ...
    # ── Painel 1: Preço + EMAs + Bollinger + Fibonacci ────────────────────────
    ax1.set_facecolor('#fafafa')

    # Bollinger preenchimento discreto
    if bb_low is not None and bb_up is not None:
        ax1.fill_between(close.index, bb_low, bb_up,
                         alpha=0.08, color='gray', zorder=0, label='Bollinger')

    # Fibonacci linhas horizontais
    for nivel, preco_fib in fib_levels.items():
        cor = fib_colors[nivel]
        ax1.axhline(preco_fib, color=cor, linestyle='--', linewidth=1, alpha=0.65, zorder=1)
        ax1.text(close.index[-1], preco_fib, f' Fib {nivel}',
                 fontsize=7.5, color=cor, va='center',
                 bbox=dict(boxstyle='round,pad=0.25', facecolor='white', edgecolor=cor, alpha=0.75))

    # Zona de Ouro (61.8%)
    f618 = fib_levels['61.8%']
    ax1.axhspan(f618 * 0.99, f618 * 1.01,
                alpha=0.15, color='#2ecc71', zorder=0)

    # Linha de preço
    ax1.plot(close.index, close.values, label='Close',
             color='#1E1E1E', linewidth=2, zorder=5)

    # EMAs
    if ema20  is not None: ax1.plot(close.index, ema20,  label='EMA20',  color='#2962FF', linewidth=1.5, alpha=0.9)
    if ema50  is not None: ax1.plot(close.index, ema50,  label='EMA50',  color='#FF6D00', linewidth=1.5, alpha=0.8)
    if ema200 is not None: ax1.plot(close.index, ema200, label='EMA200', color='#00695C', linewidth=2.0, alpha=0.7)

    rsi_atual  = round(float(rsi_s.iloc[-1]),  1) if rsi_s  is not None else 0
    stoch_atual = round(float(st_s.iloc[-1]),  1) if st_s   is not None else 0
    is_val = ((100 - rsi_atual) + (100 - stoch_atual)) / 2

    ax1.set_title(
        f'{ticker} — {etf_nome}  |  I.S.: {is_val:.0f}  |  {status}  |  Fib próx.: {nivel_prox}',
    ...
    ax1.set_ylabel('Preço (USD)', fontsize=9)
    ax1.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'${x:,.2f}'))

    # ── Painel 2: RSI ─────────────────────────────────────────────────────────
    ax2.set_facecolor('#fafafa')
    if rsi_s is not None:
        ax2.plot(close.index, rsi_s, color='#FF6F00', linewidth=1.5, label='RSI14')
        ax2.axhline(30, color='#F44336', linestyle='--', linewidth=1, alpha=0.7)
        ax2.axhline(70, color='#4CAF50', linestyle='--', linewidth=1, alpha=0.7)
        ax2.fill_between(close.index, 0, 30,  alpha=0.15, color='#F44336')
        ax2.fill_between(close.index, 70, 100, alpha=0.15, color='#4CAF50')
        # Highlight RSI atual
        ax2.text(close.index[-1], rsi_s.iloc[-1], f'  {rsi_atual:.0f}',
    ...
    if st_s is not None:
        ax3.plot(close.index, st_s, color='#9C27B0', linewidth=1.5, label='Stoch %K')
        ax3.axhline(20, color='#F44336', linestyle='--', linewidth=1, alpha=0.7)
        ax3.axhline(80, color='#4CAF50', linestyle='--', linewidth=1, alpha=0.7)
        ax3.fill_between(close.index, 0, 20,  alpha=0.15, color='#F44336')
        ax3.fill_between(close.index, 80, 100, alpha=0.15, color='#4CAF50')
        ax3.text(close.index[-1], st_s.iloc[-1], f'  {stoch_atual:.0f}',
                 fontsize=8, color='#9C27B0', va='center')
    ax3.set_ylabel('Stoch %K', fontsize=9)
    ax3.set_ylim(0, 100)
